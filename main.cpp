...
void do_video_or_cam(trtNet &net)
{
	...
	while (1)
	{
		...
		if (pthread_create(&detect_thread, 0, detect_in_thread, &net)) error("Thread creation failed");//?造一??程?行网?


		pthread_join(fetch_thread, 0);//塞入?程
...
void do_image(trtNet &net)
{
	...
	while(1)
	{
		...
		if (pthread_create(&detect_thread, 0, detect_image_in_thread, &net)) error("Thread creation failed");//?造一??程?行网?
...
int main(int argc, char* argv[])
{
	...
	string calibFileList = parser::getStringValue("calib");  //校?文件的名字 txt
	...
	if (calibFileList.length() > 0 && mode == "int8")
	{
		...
		while (getline(file, strLine))
		{
			...
			auto data = prepareImage(img);  //???608*608的?据
	...
	else if (!inputstream.compare("image"))
	{
		...
		else
		{
			...
			if (inputFileName.find(".jpg")!= inputFileName.npos || inputFileName.find(".png") != inputFileName.npos)//如果是???片
			{
